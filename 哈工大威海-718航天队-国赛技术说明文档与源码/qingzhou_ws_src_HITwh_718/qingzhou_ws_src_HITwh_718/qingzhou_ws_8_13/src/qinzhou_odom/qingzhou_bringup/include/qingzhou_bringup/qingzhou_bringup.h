...
typedef struct sMartcarControl{
  ...
  float TargetAngle;        //期望角度，原本是int
  float TargetSpeed;        //期望速度，原本是int
...
public:
     ...
     float tempaccelX,tempaccelY,tempaccelZ;     //加速度缓存区
     float tempgyroX,tempgyroY,tempgyroZ;        //角速度缓存区
     float tempmagX,tempmagY,tempmagZ;           //磁力计缓存区
