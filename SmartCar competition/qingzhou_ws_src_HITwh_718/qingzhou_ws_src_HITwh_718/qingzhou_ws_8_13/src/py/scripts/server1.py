...
# 获取位置子线程同样使用listen阻塞进程，有连接后则0.1s发送一次从参数服务器读到的位置信息
...
class Pos_Send_Object:
    def __init__(self, S_addr_port=(local_addr, 7777)):  # S_addr_port 是一个元组  ，self初始化对应的实例变量
        self.addr_port = S_addr_port  # 地址绑定，addr_port 对应的端口号
        self.Set_Socket(self.addr_port)  # 建立连接，调用一个函数
        # 设置套接字
            ...
            if message != "end!!!":
                if message != "___end":
                    if message == "camera":
                        ...
                        rospy.set_param("goal_index", 1)
                        # rospy.set_param("pathpointchange",True)
                        print("NOW GOING GAOL1...")

                    elif message == "__cmd2":
                        rospy.set_param("goal_index", 2)
                        # rospy.set_param("pathpointchange",True)
                        print("NOW GOING GAOL2...")
                    elif message == "__cmd3":
                        rospy.set_param("goal_index", 3)
                        # rospy.set_param("pathpointchange",True)
                        print("NOW GOING GAOL3...")
                    elif message == "___cmd":
                        com = command()
                        ...
                        if data[6:] == "change":
                            ...
                            os.system("rosrun amcl initial_pose")
                        elif data[6:] == "exit":
                            rospy.set_param("exit", True)
                    else:
                    ...
                    # server.close()
                    server.close()
                    thread1.pos.server.shutdown(2)
                    thread1.pos.server.close()
                    break
            else:
                import os
